...
s.listed_names={511002974,511002975}
...
function s.filter(c,tid)
	--if not c:IsCode(511002974) or c:GetFlagEffect(id)>0 then return false end
...
function s.actg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return #g>0 and (Duel.GetLocationCount(tp,LOCATION_SZONE)>0 or g:FilterCount(Card.IsLocation,nil,LOCATION_SZONE)>0) end
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,#g,0,0)
end
function s.acop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(aux.TRUE,tp,LOCATION_ONFIELD,0,nil)
	if Duel.Destroy(g,REASON_EFFECT)>0 then
		local tc=Duel.CreateToken(tp, 511002975)
		if tc then
