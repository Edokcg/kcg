...
function s.scfilter(c,sc,tp)
	return c:IsType(TYPE_SYNCHRO,sc,SUMMON_TYPE_SYNCHRO,tp) and c:IsRace(RACE_DRAGON,sc,SUMMON_TYPE_SYNCHRO,tp)
end

function s.lvfilter(c,lc,sumtype,tp)
	return c:IsType(TYPE_SYNCHRO,lc,sumtype,tp) and c:IsRace(RACE_DRAGON,lc,sumtype,tp)
end
function s.lcheck(g,lc,sumtype,tp)
	return g:IsExists(s.lvfilter,1,nil,lc,sumtype,tp)
end
