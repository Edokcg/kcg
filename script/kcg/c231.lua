...
function s.initial_effect(c)
	...
	e3:SetCondition(function(e,tp) return s.goldsarccon(e) end)
...
function s.marshsptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(s.marshspfilter,tp,LOCATIONS_REMOVED_HAND_DECK_GRAVE,0,1,e:GetHandler(),e,tp) 
...
function s.repfilter(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE)
		and not c:IsReason(REASON_REPLACE) and c:IsReason(REASON_EFFECT) and c:GetReasonPlayer()~=tp
end
