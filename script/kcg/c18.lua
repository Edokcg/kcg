...
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	  local lp1=Duel.GetLP(1-tp)-Duel.GetLP(tp)
	  local lp2=Duel.GetLP(tp)-Duel.GetLP(1-tp)
	  local dam=0
	  if lp1>0 then dam=lp1 else dam=lp2 end 
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	local p=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER)
	  local lp1=Duel.GetLP(1-p)-Duel.GetLP(p)
	  local lp2=Duel.GetLP(p)-Duel.GetLP(1-p)
	  local dam=0
	  if lp1>0 then dam=lp1 else dam=lp2 end 
...
function s.con(e,tp,eg,ep,ev,re,r,rp)
	...
	return e:GetHandler()==Duel.GetAttacker() or e:GetHandler()==Duel.GetAttackTarget() and Duel.GetLP(p)>Duel.GetLP(1-p)
end
function s.op(e,tp,eg,ep,ev,re,r,rp)
	local p=e:GetHandler():GetControler()
	Duel.Damage(1-tp,Duel.GetLP(p)-Duel.GetLP(1-p),REASON_EFFECT)
end
