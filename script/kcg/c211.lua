...
function s.initial_effect(c)
   ...
   e1:SetCondition(function(e,tp,eg,ep,ev,re,r,rp)
      ...
      return Duel.GetFlagEffect(ttp, 112) == 0 and rp == 1 - ttp 
   ...
   e3:SetCondition(function(e,tp,eg,ep,ev,re,r,rp)
      local ttp = e:GetOwner():GetControler()
      return Duel.GetFlagEffect(ttp, 113) == 0 and ep == 1 - ttp 
   ...
   e4:SetCondition(function(e,tp,eg,ep,ev,re,r,rp)
      local ttp = e:GetOwner():GetControler()
      return Duel.GetFlagEffect(ttp, 114) == 0 and ep == 1 - ttp
   ...
   e5:SetProperty(EFFECT_FLAG_UNCOPYABLE+EFFECT_FLAG_IGNORE_IMMUNE+EFFECT_FLAG_DAMAGE_CAL+EFFECT_FLAG_DAMAGE_STEP)
   e5:SetType(EFFECT_TYPE_CONTINUOUS+EFFECT_TYPE_FIELD)
   e5:SetCode(EVENT_SSET)
   e5:SetCondition(function(e,tp,eg,ep,ev,re,r,rp)
      local ttp = e:GetOwner():GetControler()
      return Duel.GetFlagEffect(ttp, 115) == 0 and ep == ttp
      and Duel.GetMatchingGroupCount(s.AIcounter4, ttp, LOCATION_HAND + LOCATION_SZONE, 0, nil) > 0
      and Duel.GetFieldGroupCount(ttp, LOCATION_DECK, 0) > 0
   end)
   e5:SetOperation(s.reverse_chain2)
   Duel.RegisterEffect(e5, 0)
   local e6=e5:Clone()
   e6:SetCode(EVENT_CHAINING)
   e6:SetCondition(function(e,tp,eg,ep,ev,re,r,rp)
      local ttp = e:GetOwner():GetControler()
      return Duel.GetFlagEffect(ttp, 115) == 0 and rp == ttp
      and Duel.GetMatchingGroupCount(s.AIcounter4, ttp, LOCATION_HAND + LOCATION_SZONE, 0, nil) > 0
      and Duel.GetFieldGroupCount(ttp, LOCATION_DECK, 0) > 0
   end)
...
function s.op(e, tp, eg, ep, ev, re, r, rp)
   local ttp = e:GetOwner():GetControler()
   local code={511001126,55144522,62015408,58851034,14558127,59438930,67750322,73642296,77538567,10045474,97268402,511002565,37742478,18964575,19665973,27204311,55063751,23434538}
...
function s.reverse_chain(e,tp,eg,ep,ev,re,r,rp)
   local code={62015408,58851034,14558127,59438930,67750322,73642296,77538567,10045474,97268402}
   local ttp = e:GetOwner():GetControler()
   s.treverse(0,code,e,ttp,eg,ep,ev,re,r,rp)
end
function s.reverse_attack(e,tp,eg,ep,ev,re,r,rp)
   local code={511002565,37742478,18964575,19665973}
   local ttp = e:GetOwner():GetControler()
...
function s.reverse_chain2(e,tp,eg,ep,ev,re,r,rp)
   local code={511001126,55144522}
   local ttp = e:GetOwner():GetControler()
   s.treverse(3,code,e,ttp,eg,ep,ev,re,r,rp)
end

function s.treverse(case,code,e,tp,eg,ep,ev,re,r,rp)
   local event=e:GetCode()
   local reset = RESET_PHASE+PHASE_MAIN1+PHASE_MAIN2+PHASE_BATTLE+PHASE_DAMAGE+PHASE_END+RESET_EVENT+RESET_CHAIN
   ...
   for _, code1 in ipairs(code) do
      ...
      if Duel.IsExistingMatchingCard(s.AIcounter2,tp,loc,0,1,nil,code1) and not Duel.IsExistingMatchingCard(s.AIcounter,tp,loc,0,1,nil,code1) then
         ...
         if g:IsOriginalType(TYPE_SPELL+TYPE_TRAP) then
            ...
            if code1==511001126 then 
               local draw=Duel.CreateToken(tp, 55144522)
               Duel.DisableShuffleCheck()
               Duel.SendtoDeck(draw,tp,0,REASON_RULE)
            end
         else
            te=tae
            -- if code1==62015408 then chkcon=mt.condition(ae,tp,eg,ep,ev,re,r,rp) and mt.cost(ae,tp,eg,ep,ev,re,r,rp,0) and mt.target(ae,tp,eg,ep,ev,re,r,rp,0) end
            ...
            if code1==62015408 then
               local codetoken = {70369116,37818794,60461804,50277355,13117073,58004362,19324993,1948619}
               for _, code2 in ipairs(codetoken) do
                  if Duel.IsExistingMatchingCard(Card.IsCode,1-tp,LOCATION_EXTRA,0,1,nil,code2) then
                     local token = Duel.CreateToken(tp, code2)
         ...
         if event == EVENT_CHAINING then
            if code1==59438930 then
               if not re:GetHandler():IsDestructable() then goto continue end
            end
         end
         ...
         Duel.RegisterFlagEffect(tp,112+case,reset,0,1)
         break
      end
