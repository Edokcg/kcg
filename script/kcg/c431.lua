...
function s.attachfilter(c,xc,tp)
	return c:IsSpellTrap() and (c:IsCanBeXyzMaterial(xc,tp,REASON_EFFECT) or (c:IsAbleToRemove() and xc:IsAbleToRemove()))
end
...
function s.attachop(e,tp,eg,ep,ev,re,r,rp)
	...
	if #tg>0 then
        local b1=tg:FilterCount(Card.IsAbleToRemove,c)==#tg and c:IsAbleToRemove()
        local b2=tg:FilterCount(Card.IsCanBeXyzMaterial,c,c,tp,REASON_EFFECT)==#tg
...
function s.atop(e,tp,eg,ep,ev,re,r,rp)
    ...
    if not tg or #tg<1 or not (c:GetPreviousLocation()==LOCATION_REMOVED and c:GetFlagEffect(id)>0) then return end
    local tg2=tg:Filter(function(tc) return tc:GetFlagEffect(id)>0 end, c)
    if #tg2>0 then Duel.Overlay(c,tg2) end
