...
function s.cfilter(c)
	return c:IsType(TYPE_FUSION+TYPE_SYNCHRO+TYPE_XYZ+TYPE_LINK) and c:IsAbleToExtraAsCost()
end
function s.spfilter(c,e,tp,g)
	return g:IsExists(s.chkfilter,1,nil,c) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function s.chkfilter(sc,c)
	local tpe=TYPE_FUSION+TYPE_SYNCHRO+TYPE_XYZ+TYPE_LINK
...
function s.filter(c,sc)
	return c:IsRace(sc:GetRace()) and c:IsAttribute(sc:GetAttribute())
		and c:IsType(sc:GetType()&(TYPE_FUSION+TYPE_SYNCHRO+TYPE_XYZ+TYPE_LINK))
end
