...
s.listed_series={SET_MEKLORD_EMPEROR,0x9013}
...
function s.filter(c)
	return c:IsSetCard(0x9013) and c:IsMonster()
end
...
function s.codefilter(c,code)
	return c:IsOriginalCode(code) and c:IsSetCard(0x9013)
end
...
function s.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(s.filterr,tp,LOCATION_HAND+LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,5,tp,LOCATION_HAND+LOCATION_DECK)
end
function s.filterr(c)
	return c:IsSetCard(0x9013)
end
function s.spop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINTMSG_TOGRAVE,tp,aux.Stringid(id,0))
	local spg=Duel.SelectMatchingCard(tp,s.filterr,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil)
	if #spg>0 then
