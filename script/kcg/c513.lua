...
function s.xyzcondition85(e, tp, eg, ep, ev, re, r, rp)
    ...
    and Duel.GetFlagEffect(tp, 90999980) ~= 0
...
function s.KDraw(ttp, count)
    ...
    if Duel.GetTurnCount() ~= 1 and Duel.GetMatchingGroupCount(nil, ttp, LOCATION_DECK, 0, nil) > 0 and
        (Duel.GetFlagEffect(ttp, 90999980) ~= 0 and Duel.GetFlagEffect(ttp, 90999980) < 3) then

        if Duel.SelectYesNo(ttp, aux.Stringid(123106, 8)) then
            local off = 1
            local ops = {}
            local opval = {}
            ops[off] = aux.Stringid(826, 11)
            opval[off - 1] = 1
            off = off + 1
            if Duel.GetMatchingGroupCount(Card.IsCode, ttp, LOCATION_DECK + LOCATION_HAND + LOCATION_ONFIELD + LOCATION_GRAVE + LOCATION_REMOVED, 0, nil, 57734012) < 4 
            and Duel.GetMatchingGroupCount(Card.IsCode, ttp, LOCATION_HAND, 0, nil, 57734012) == 0  then
