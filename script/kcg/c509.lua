...
function s.thfilter32(c,e,tp,tc)
	local setcard=tc:GetSetCard()
	return c:IsMonster() and (c:IsSetCard(0x525) or c:IsSetCard(0x507) or c:IsSetCard(0x557) or c:IsSetCard(0x50d)) and c:IsCanBeSpecialSummoned(e,0,tp,false,false) and c:IsSetCard(setcard)
end
function s.thfilter3(c,e,tp)
	return c:IsFaceup() and c:IsMonster() and (c:IsSetCard(0x525) or c:IsSetCard(0x507) or c:IsSetCard(0x557) or c:IsSetCard(0x50d)) and c:IsAbleToGrave() and Duel.GetLocationCount(tp,LOCATION_MZONE)>-1
	and Duel.IsExistingMatchingCard(s.thfilter32,tp,LOCATION_DECK|LOCATION_GRAVE,0,1,nil,e,tp,c)
end
function s.thtg3(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chk==0 then return Duel.IsExistingMatchingCard(s.thfilter3,tp,LOCATION_MZONE,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_TOGRAVE,nil,1,tp,LOCATION_MZONE)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK|LOCATION_GRAVE)
end
function s.thop3(e,tp,eg,ep,ev,re,r,rp,chk)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g=Duel.SelectMatchingCard(tp,s.thfilter3,tp,LOCATION_MZONE,0,1,1,nil,e,tp)
	if #g>0 and Duel.SendtoGrave(g,REASON_EFFECT)>0 and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local ag=Duel.SelectMatchingCard(tp,s.thfilter32,tp,LOCATION_DECK|LOCATION_GRAVE,0,1,1,nil,e,tp,g:GetFirst())
...
function s.dcfilter(c)
	return c:IsMonsterCard() and c:IsDestructable()
end
function s.mfilter(c)
	return c:IsMonster() and c:IsSetCard(SET_MEKLORD)
	and c:GetEquipGroup():IsExists(s.dcfilter,1,nil)
end
function s.dircost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(aux.FaceupFilter(s.mfilter),tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectMatchingCard(tp,aux.FaceupFilter(s.dcfilter),tp,LOCATION_SZONE,0,1,1,nil)
...
function s.thfilter2(c)
	return c:IsCode(404) and c:IsAbleToHand()
end
function s.thfilter4(c)
	return c:IsCode(404) and c:IsSSetable()
end
