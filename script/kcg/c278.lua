...
function s.fcheck(tp,sg,fc,mg)
	if sg:IsExists(Card.IsHasEffect,1,nil,id) then
		return sg:IsExists(s.filterchk,1,nil,fc,SUMMON_TYPE_FUSION,tp) end
	return true
end
function s.filterchk(c,scard,sumtype,playerid)
	return c:IsSetCard(0x10af,scard,sumtype,playerid) and not c:IsHasEffect(id)
end
