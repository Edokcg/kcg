...
function s.filter(c,tp)
	...
	and Duel.IsExistingMatchingCard(s.filter2,tp,LOCATION_GRAVE,0,1,c,c:GetAttribute())
end
...
function s.lspfilter(c,tp)
	return c:IsSetCard(0x577) and c:IsType(TYPE_LINK) and c:IsLinkSummonable(nil,Duel.GetFieldGroup(tp,LOCATION_GRAVE,0))
end
function s.operation(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.BreakEffect()
	local g=Duel.GetMatchingGroup(s.lspfilter,tp,LOCATION_EXTRA,0,nil,tp)
	if #g>0 then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local sg=g:Select(tp,1,1,nil)
		Duel.LinkSummon(tp,sg:GetFirst(),nil,Duel.GetFieldGroup(tp,LOCATION_GRAVE,0))
	end
end