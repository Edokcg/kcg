...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if re:GetHandler():IsAbleToRemove() and re:GetHandler():IsRelateToEffect(re) and e:GetHandler():IsAbleToRemove() then
		Duel.SetPossibleOperationInfo(0,CATEGORY_REMOVE,eg,2,0,0)
	end
end
...
function s.atop(e,tp,eg,ep,ev,re,r,rp)
	...
	if not (c:GetPreviousLocation()==LOCATION_REMOVED and c:GetFlagEffect(id)>0 and tc and tc:GetFlagEffect(id)>0) then return end
	Duel.Overlay(c,tc)
