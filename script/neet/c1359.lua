...
function s.initial_effect(c)
	...
	e0:SetCondition(s.fucon)
	e0:SetOperation(s.fuop)
	c:RegisterEffect(e0)
	--spsummon condition
	...
	e2:SetDescription(aux.Stringid(1359,1))
...
function s.fufilter(c,fc)
	return c:IsCanBeFusionMaterial(fc,SUMMON_TYPE_SPECIAL) and c:IsReleasable()
end
function s.fucon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local mg=Duel.GetMatchingGroup(s.fufilter,tp,LOCATION_MZONE,0,c,c)
	return c:CheckFusionMaterial(mg,nil,tp|0x200)
end
function s.fuop(e,tp,eg,ep,ev,re,r,rp,c)
	local mg=Duel.GetMatchingGroup(s.fufilter,tp,LOCATION_MZONE,0,c,c)
