...
function s.initial_effect(c)
	...
	e03:SetTarget(s.syntg)
	e03:SetValue(1)
	e03:SetOperation(s.synop)
	...
	e3:SetDescription(aux.Stringid(1556,0))
...
function s.syntg(e,syncard,f,min,max)
	...
	local mg=Duel.GetMatchingGroup(s.synfilter,tp,LOCATION_MZONE+LOCATION_HAND,LOCATION_MZONE,c,syncard,c,f)
...
function s.lvop(e,tp,eg,ep,ev,re,r,rp)
	...
	if sg:GetCount()>0 and Duel.SelectYesNo(tp,aux.Stringid(1556,2)) then
...
function s.lvop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.SelectOption(tp,aux.Stringid(1556,1),aux.Stringid(1556,2))==1 then
		lvl=-e:GetLabel()
	end
	local c=e:GetHandler()
...
function s.hlvop(e,tp,eg,ep,ev,re,r,rp)
	local hg=eg:Filter(s.lvfilter,nil,tp)
	local tc=hg:GetFirst()
	local lvl=e:GetLabel()
	while tc do
