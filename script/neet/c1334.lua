...
function s.penfilter(c,e,tp,lscale,rscale)
	return c:IsSetCard(0xd8) and Pendulum.Filter(c,e,tp,lscale,rscale)
end
...
function s.thcfilter(c,tp)
	return c:IsPreviousSetCard(0xd8) and c:IsType(TYPE_PENDULUM) 
		and c:IsPreviousControler(tp) and c:IsPreviousPosition(POS_FACEUP)
		and c:IsPreviousLocation(LOCATION_PZONE)
end
