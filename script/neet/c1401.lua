...
function s.costfilter(c)
	return c:IsAttribute(ATTRIBUTE_DARK) and c:IsDefenseBelow(3000)
end
function s.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if g:GetFirst():IsDefenseAbove(2000) then e:SetLabel(1) end
	Duel.Release(g,REASON_COST)
end
function s.tgfilter(c,lb)
	return c:IsFaceup() and c:IsAbleToRemove() and (c:IsNormalSpell() or c:IsNormalTrap() or (c:IsQuickPlaySpell() and lb==1) or (c:IsCounterTrap() and lb==1))
end
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local g=Duel.GetMatchingGroup(s.tgfilter,tp,0,LOCATION_ONFIELD,nil,e:GetLabel())
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,g,#g,0,0)
end
function s.filter(c,lb)
	return (c:IsNormalSpell() or c:IsNormalTrap() or (c:IsQuickPlaySpell() and lb==1) or (c:IsCounterTrap() and lb==1)) and c:IsAbleToRemove()
end
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if #conf>0 then
		...
		local dg=conf:Filter(s.filter,nil,e:GetLabel())
	...
	c:RegisterEffect(e3)
	local lb=e:GetLabel()
	aux.RegisterClientHint(e:GetHandler(),nil,tp,0,1,aux.Stringid(id,lb),nil)
end
...
function s.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	local dg=hg:Filter(s.filter,nil,1)
	Duel.Remove(dg,POS_FACEUP,REASON_EFFECT)
	Duel.ShuffleHand(ep)
end
function s.desop2(e,tp,eg,ep,ev,re,r,rp)
	...
	if #hg==0 then return end
	Duel.ConfirmCards(1-ep,hg)
	local dg=hg:Filter(s.filter,nil,0)
	Duel.Remove(dg,POS_FACEUP,REASON_EFFECT)
	Duel.ShuffleHand(ep)
end
