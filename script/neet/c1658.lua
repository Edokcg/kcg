...
function s.filter(c,tp)
	return c:IsSetCard(0x11b) and c:IsType(TYPE_MONSTER) and c:IsAbleToHand() and Duel.IsExistingMatchingCard(s.thfilter3,tp,LOCATION_DECK,0,1,c)
end
function s.thfilter3(c)
	return c:IsSetCard(0x11b) and (c:IsType(TYPE_SPELL) or c:IsType(TYPE_TRAP)) and c:IsAbleToHand()
end
