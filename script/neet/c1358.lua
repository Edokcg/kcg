...
function s.initial_effect(c)
	...
	e1:SetDescription(aux.Stringid(1358,0))
	...
	e03:SetCondition(s.ctcon2)
	e03:SetValue(s.efilter)
...
function s.ctcon(e)
	return e:GetHandler():GetOverlayGroup():IsExists(s.ctfilter,1,nil) and e:GetHandlerPlayer()==Duel.GetTurnPlayer()
end
function s.ctcon2(e)
	return e:GetHandler():GetOverlayGroup():IsExists(s.ctfilter,1,nil) and e:GetHandlerPlayer()~=Duel.GetTurnPlayer()
end
...
function s.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.SelectEffectYesNo(tp,e:GetHandler(),aux.Stringid(1358,1)) then
		local g=eg:Filter(s.repfilter,1,nil,tp)
