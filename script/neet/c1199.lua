...
function s.initial_effect(c)
	--Link Summon
	c:EnableReviveLimit()
	Link.AddProcedure(c,aux.FilterBoolFunctionEx(Card.IsSetCard,0x8d),1)
--spsummon
...
function s.filter1(c,e,tp)
	...
	return (#pg<=0 or (#pg==1 and pg:IsContains(c))) and c:IsFaceup() and c:IsSetCard(0x8d)
...
function s.filter2(c,e,tp,mc,code,pg)
	return  c:IsType(TYPE_XYZ) and c:IsSetCard(0x8d)  and mc:IsCanBeXyzMaterial(c,tp)
