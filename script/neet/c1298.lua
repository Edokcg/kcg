...
function s.matfilter(c,lc,sumtype,tp)
	return c:IsSetCard(0x114,lc,sumtype,tp) and not c:IsType(TYPE_LINK,lc,sumtype,tp)
end
...
function s.thfilter(c,tp)
	return c:IsSetCard(0x114) and c:IsMonster() and c:IsAbleToHand() and not Duel.IsExistingMatchingCard(aux.FaceupFilter(Card.IsRace,c:GetRace()),tp,LOCATION_MZONE,0,1,nil)
end
