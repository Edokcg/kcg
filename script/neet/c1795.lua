...
s.listed_names={id,94585852,63883999}
...
function s.spop(e,tp,eg,ep,ev,re,r,rp,c)
 ...
 function s.thfilter(c)
	return c:IsSetCard(0x45) and c:IsMonster() and not c:IsCode(id) and c:IsAbleToHand()
end
...
function s.thop(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.IsExistingMatchingCard(s.filter,tp,LOCATION_ONFIELD|LOCATION_GRAVE,0,1,nil) then
		Duel.BreakEffect()
		Duel.Damage(1-tp,1000,REASON_EFFECT)
	end
end
function s.filter(c)
	return c:IsFaceup() and c:IsCode(94585852) or c:IsCode(63883999)
end