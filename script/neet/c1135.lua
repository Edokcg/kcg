...
function s.filter1(c,e,tp)
	...
	return (#pg<=0 or (#pg==1 and pg:IsContains(c))) and c:IsRank(8) and c:IsFaceup()
...
function s.filter2(c,e,tp,mc,rk,pg)
	...
	return mc:IsType(TYPE_XYZ,c,SUMMON_TYPE_XYZ,tp) and c:IsSetCard(0x107b) and mc:IsCanBeXyzMaterial(c,tp)
...
function s.drfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x107b) and c:IsRank(8)
end
